...
V = np.sqrt(Ux**2 + Uy**2)
fig, axs = plt.subplot_mosaic([['V', 'P',]], figsize=(12, 6))
# Tracé de Ux en fonction de y pour une position x fixe
a = np.linspace(0, L, n)  # Grille x avec N points
b = np.linspace(0, l, m)  # Grille y avec M points

# Meshgrid pour les calculs
A, B = np.meshgrid(a, b)
ux = ((P2 - P1) / (2 * C * L)) * (B ** 2 - l * B)
p = ((P2 - P1) / L) * A + P1
v = np.abs(ux)
error_V_percentage = np.where(np.abs(v[:, 1]) > 1e-12,
                               (np.abs(V[:, 1] - v[:, 1]) / np.abs(v[:, 1]) )* 100,
                               0)

error_P_percentage = np.where(np.abs(p[0, :]) > 1e-12,
                               (np.abs(P[0, :] - p[0, :]) / np.abs(p[0, :]) )* 100,
                               0)


axs['V'].plot(b, error_V_percentage, label="Erreur V(m/s) (%)", c='g', linestyle='--')
axs['V'].legend()
# Tracé de P en fonction de y pour une position x fixe

axs['P'].plot(a, error_P_percentage, label="Erreur P(Pa) (%)", c='g', linestyle='--')
axs['P'].legend()
